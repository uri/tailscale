...
type NetfilterRunner interface {
	...
	// AddMasqueradeRule adds a rule to the nat/POSTROUTING chain to SNAT
	// traffic destined for dst to src.
	AddSNATRuleForDst(src, dst netip.Addr) error

	// DNATNonTailscaleTraffic adds a rule to the nat/PREROUTING chain to DNAT all traffic
	// inbound from a non-tailscale interface to the given destination.
	DNATNonTailscaleTraffic(tun string, dst netip.Addr) error

	// ClampMSSToPMTU adds a rule to the mangle/FORWARD chain to clamp MSS for traffic
	// destined for the tailscale0 interface.
