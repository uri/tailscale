...
func NewRestartManagerSession(logf logger.Logf) (RestartManagerSession, error) {
	...
	if err := rmStartSession(&result.session, 0, unsafe.SliceData(sessionKeyBuf[:])); err != nil {
		return nil, err
	}
...
func (up *UniqueProcess) AsRestartableProcess() (*RestartableProcess, error) {
	...
	// in order for ProcessImageName to be able to work from within a privileged Windows Installer process.
	...
	var rmFlags uint32
	hr := getApplicationRestartSettings(h, unsafe.SliceData(cmdLine[:]), &cmdLineLen, &rmFlags)
...
// Close will invoke (*RestartableProcess).Close on every value in rps.
...
// Terminal Services (RDP) sessions and NT logon sessions.
...
// SetArgs converts args to a string quoted as necessary to satisfy Win32
// command line arguments, and sets cli.Args to that string.
...
// CreateProcess* family of APIs, as pointers to C-style UTF-16 strings.
func (cli *CommandLineInfo) Resolve() (exePath *uint16, cmdLine *uint16, err error) {
	// Resolve cmdLine first since that also does a Validate.
	cmdLine, err = cli.resolveArgsAsUTF16Ptr()
	if err != nil {
		return nil, nil, err
	}
...
func (cli *CommandLineInfo) resolveArgsAsUTF16Ptr() (*uint16, error) {
...
// reside inside the session identified by sessID, with the token whose logon
// is associated with sessID. Its environment will be inherited from the
// session token's environment.
func StartProcessInSession(logf logger.Logf, sessID SessionID, cmdLineInfo CommandLineInfo) error {
	...
	return startProcessInSessionInternal(l, sessID, cmdLineInfo)
}

func startProcessInSessionInternal(logf logger.Logf, sessID SessionID, cmdLineInfo CommandLineInfo) error {
	logf("Validating cmdLineInfo")
	if err := cmdLineInfo.Validate(); err != nil {
		return err
	}

	logf("Obtaining token for TS session %d", sessID.TSSession)
	...
	logf("Obtaining logon session LUID")
	logonSessionID, err := LogonSessionID(token)
	if err != nil {
		return err
	}
	if logonSessionID != sessID.LogonSession {
		return windows.ERROR_BAD_LOGON_SESSION_STATE
	}

	logf("Resolving exe and command line paths")
	exePath16, commandLine16, err := cmdLineInfo.Resolve()
	if err != nil {
		return fmt.Errorf("(*CommandLineInfo).Resolve(): %w", err)
	}

	logf("Resolving working directory")
	wd16, err := windows.UTF16PtrFromString(filepath.Dir(cmdLineInfo.ExePath))
	if err != nil {
		return fmt.Errorf("UTF16PtrFromString(wd): %w", err)
	}

	logf("Building environment block")
	env, err := token.Environ(false)
	if err != nil {
		return fmt.Errorf("token environment: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("EnableCurrentThreadPrivileges(%#v): %w", privNames, err)
	}
	defer dropPrivs()

	logf("Calling CreateProcessAsUser")
	...
	if err := windows.CreateProcessAsUser(token, exePath16, commandLine16, nil, nil,
		false, windows.CREATE_UNICODE_ENVIRONMENT, env16, wd16, &si, &pi); err != nil {
		return fmt.Errorf("CreateProcessAsUser: %w", err)
	}

	logf("Closing handles")
